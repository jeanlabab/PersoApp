...
class TexturePlugin : public Plugin {
	...
	typedef Texture<int> *
		(*CreateIntFunc)(const Transform &shader2world, const TextureParams &);
	typedef Texture<double> *
		(*CreateDoubleFunc)(const Transform &shader2world, const TextureParams &);
public:
	TexturePlugin( const string &name ): Plugin( name )
	{
		...
		CreateIntTex =
			(CreateIntFunc)(GetSymbol("CreateIntTexture"));
		CreateDoubleTex =
			(CreateDoubleFunc)(GetSymbol("CreateDoubleTexture"));
	}
	CreateFloatFunc CreateFloatTex;
	CreateSpectrumFunc CreateSpectrumTex;
	CreateIntFunc CreateIntTex;
	CreateDoubleFunc CreateDoubleTex;
};
...
COREDLL Reference<Texture<int> > MakeIntTexture(const string &name,
		const Transform &tex2world, const TextureParams &tp) {
	TexturePlugin *plugin = GetPlugin<TexturePlugin>(name, texturePlugins,
		PluginSearchPath);
	if (plugin) {
		Reference<Texture<int> > ret =
			plugin->CreateIntTex(tex2world, tp);
		tp.ReportUnused();
		return ret;
	}
	return NULL;
}
COREDLL Reference<Texture<double> > MakeDoubleTexture(const string &name,
		const Transform &tex2world, const TextureParams &tp) {
	TexturePlugin *plugin = GetPlugin<TexturePlugin>(name, texturePlugins,
		PluginSearchPath);
	if (plugin) {
		Reference<Texture<double> > ret =
			plugin->CreateDoubleTex(tex2world, tp);
		tp.ReportUnused();
		return ret;
	}
	return NULL;
