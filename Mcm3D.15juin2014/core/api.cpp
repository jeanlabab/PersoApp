...
struct GraphicsState {
	...
	map<string, Reference<Texture<Spectrum> > >
		spectrumTextures;
	map<string, Reference<Texture<int> > >
		intTextures;
	map<string, Reference<Texture<double> > >
		doubleTextures;
...
COREDLL void pbrtTexture(const string &name,
	...
	TextureParams tp(params, params,
			     ...
			     graphicsState.spectrumTextures,
        	             graphicsState.intTextures,
		             graphicsState.doubleTextures);
	if (type == "float")  {
		...
		// Create _int_ texture and store in _intTextures_
		if (graphicsState.intTextures.find(name) !=
		    graphicsState.intTextures.end())
			Warning("Texture \"%s\" being redefined", name.c_str());
		Reference<Texture<int> > it = MakeIntTexture(texname,
			curTransform, tp);
		if (it) graphicsState.intTextures[name] = it;
	}
	else if (type == "double")  {
		// Create _double_ texture and store in _doubleTextures_
		if (graphicsState.doubleTextures.find(name) !=
		    graphicsState.doubleTextures.end())
			Warning("Texture \"%s\" being redefined", name.c_str());
		Reference<Texture<double> > dt = MakeDoubleTexture(texname,
			curTransform, tp);
		if (dt) graphicsState.doubleTextures[name] = dt;
	}
	else
