...
public:
	...
	BSDF(const DifferentialGeometry &dgs,
	     const Normal &ngeom,
	     float eta = 1.f, const bool san = false, const int nums = -1, const double temp = -1);
	...
	void set_mcmNumeroDeLaSurface(const int &nums) {
	  if (nums >= 0) {
	    mcmSurfaceAvecNumero = true;
	    mcmNumeroDeLaSurface = nums;
	  }
	}
	void set_mcmTemperature(const double &temp) { mcmTemperature = temp; }
	bool get_mcmSurfaceAvecNumero(int &nums) {
	  nums = mcmNumeroDeLaSurface;
	  return mcmSurfaceAvecNumero;
	}
	double get_mcmTemperature() { return mcmTemperature; }
	// BSDF Public Data
...
private:
	...
	bool mcmSurfaceAvecNumero;
	int mcmNumeroDeLaSurface;
