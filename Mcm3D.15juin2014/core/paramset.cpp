...
ParamSet &ParamSet::operator=(const ParamSet &p2) {
	if (&p2 != this) {
		...
		for (i = 0; i < p2.doubles.size(); ++i)
			doubles.push_back(p2.doubles[i]->Clone());
...
void ParamSet::AddDouble(const string &name,
			            const double *data,
						int nItems) {
	EraseDouble(name);
	doubles.push_back(new ParamSetItem<double>(name,
											 data,
											 nItems));
}
...
bool ParamSet::EraseDouble(const string &n) {
	for (u_int i = 0; i < doubles.size(); ++i)
		if (doubles[i]->name == n) {
			delete doubles[i];
...
double ParamSet::FindOneDouble(const string &name,
                             double d) const {
	for (u_int i = 0; i < doubles.size(); ++i)
		if (doubles[i]->name == name &&
			doubles[i]->nItems == 1) {
			doubles[i]->lookedUp = true;
			return *(doubles[i]->data);
		}
	return d;
}
...
const double *ParamSet::FindDouble(const string &name,
		int *nItems) const {
	for (u_int i = 0; i < doubles.size(); ++i)
		if (doubles[i]->name == name) {
			*nItems = doubles[i]->nItems;
			doubles[i]->lookedUp = true;
			return doubles[i]->data;
		}
	return NULL;
...
#define CHECK_UNUSED(v) \
	...
	CHECK_UNUSED(textures); CHECK_UNUSED(doubles);
}
void ParamSet::Clear() {
#define DEL_PARAMS(name) \
	...
	DEL_PARAMS(textures); DEL_PARAMS(doubles);
#undef DEL_PARAMS
}
string ParamSet::ToString() const {
	...
	for (i = 0; i < doubles.size(); ++i) {
		char *bufp = buf;
		*bufp = '\0';
		ParamSetItem<double> *item = doubles[i];
		typeString = "double ";
		// Print _ParamSetItem_ declaration, determine how many to print
		int nPrint = item->nItems;
		ret += string("\"");
...
Reference<Texture<int> > TextureParams::GetIntTexture(const string &n,
		int def) const {
	string name = geomParams.FindTexture(n);
	if (name == "") name = materialParams.FindTexture(n);
	if (name != "") {
		if (intTextures.find(name) != intTextures.end())
			return intTextures[name];
		else
			Error("Couldn't find int texture named \"%s\"", n.c_str());
	}
	float val = geomParams.FindOneInt(n,
		materialParams.FindOneInt(n, def));
	return new ConstantTexture<int>(val);
}
Reference<Texture<double> > TextureParams::GetDoubleTexture(const string &n,
		double def) const {
	string name = geomParams.FindTexture(n);
	if (name == "") name = materialParams.FindTexture(n);
	if (name != "") {
		if (doubleTextures.find(name) != doubleTextures.end())
			return doubleTextures[name];
		else
			Error("Couldn't find double texture named \"%s\"", n.c_str());
	}
	float val = geomParams.FindOneDouble(n,
		materialParams.FindOneDouble(n, def));
