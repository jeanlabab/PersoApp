...
public:
	...
	virtual Point UniformVolumeSample(float u1, float u2, float u3, Normal *Ns) const {
		Severe("Unimplemented Shape::UniformVolumeSample method called");
		return Point();
	}
	virtual bool IsInVolume() const {
		Severe("Unimplemented Shape::IsInVolume method called");
		return false;
	}
	...
	virtual Point UniformVolumeSample(const Point &P,
			float u1, float u2, float u3, Normal *Ns) const {
		return UniformVolumeSample(u1, u2, u3, Ns);
	}
	virtual bool IsInVolume(Point P) const {
		return IsInVolume();
	}
	virtual float VolumeWithinClosedSurface() const {
		return float();
	}
